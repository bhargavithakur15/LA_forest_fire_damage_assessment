...
- **`LA2025_fire.js`** — Advanced Normalized Burn Ratio (NBR/dNBR) workflow with BRDF normalization and terrain illumination correction for more accurate severity mapping.
- **`LA_fire_NBR.js`** — NBR/dNBR burn severity classification with an interactive time-series chart: click any point on the map to plot its NBR trajectory over time.
- **`Accuracy_Sentinel_Landsat.js`** — Cross-sensor comparison: computes dNDVI burn severity independently from Sentinel-2 and Landsat 8, resamples both to a common 30 m grid, and compares them with an error matrix (overall agreement, producer's/consumer's accuracy, kappa). This is the script behind the ~85% cross-sensor agreement figure — it measures how well the two optical sensors agree with each other, not correctness against ground truth.
...
These scripts are written for the [Google Earth Engine Code Editor](https://code.earthengine.google.com/) (JavaScript API). `Accuracy_LAfire.js` references a project-specific geometry (`accurate_zone`) that must be defined or imported in the Code Editor before running.
